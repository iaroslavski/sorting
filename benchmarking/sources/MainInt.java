...
public final class MainInt {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = int, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("jdk", new Sortable() { public void sortArray(int[] a) { DualPivotQuicksort_jdk.sort(a, parallelism, LEFT, HIGH ); }});
        doSort("__ ", new Sortable() { public void sortArray(int[] a) { DualPivotQuicksort    .sort(a, parallelism, LEFT, HIGH ); }});
...
*/
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (int) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (int) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }

    public static void check(String name, int[] a1, int[] a2, int left, int right) {
        String message;
    ...
    public static void outArr(int[] a, int left, int right) {
        for (int i = left; i <= right; ++i) {
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final int[] test = new int[MAX_N];
    private static final int[] golden = new int[MAX_N];
