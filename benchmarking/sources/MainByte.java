...
public final class MainByte {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = byte, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("jdk", new Sortable() { public void sortArray(byte[] a) { DualPivotQuicksort_jdk.sort(a, parallelism, LEFT, HIGH ); }});
        doSort("__ ", new Sortable() { public void sortArray(byte[] a) { DualPivotQuicksort    .sort(a,              LEFT, HIGH ); }});
...
*/
    ...
    private static void reverse(byte[] a, int start, int end) {
        while (start < end) {
            byte t = a[start];
            a[start++] = a[end];
            a[end--] = t;
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (byte) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (byte) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }

    public static void check(String name, byte[] a1, byte[] a2, int left, int right) {
        String message;
    ...
    public static void outArr(byte[] a) {
        outArr(a, 0, a.length - 1);
    }

    public static void outArr(byte[] a, int left, int right) {
        for (int i = left; i <= right; ++i) {
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final byte[] test = new byte[MAX_N];
    private static final byte[] golden = new byte[MAX_N];
