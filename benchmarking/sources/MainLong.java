...
public final class MainLong {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = long, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("__ ", new Sortable() { public void sortArray(long[] a) { DualPivotQuicksort    .sort(a, parallelism, LEFT, HIGH ); }});
...
*/
    ...
    private static void reverse(long[] a, int start, int end) {
        while (start < end) {
            long t = a[start];
            a[start++] = a[end];
            a[end--] = t;
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (long) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (long) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final long[] test = new long[MAX_N];
    private static final long[] golden = new long[MAX_N];
