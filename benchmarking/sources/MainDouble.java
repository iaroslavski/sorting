...
public final class MainDouble {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = double, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("__ ", new Sortable() { public void sortArray(double[] a) { DualPivotQuicksort    .sort(a, parallelism, LEFT, HIGH ); }});
...
*/
    ...
    private static void random() {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (double) random.nextInt();
        }
    }
    ...
    private static void reverse(double[] a, int start, int end) {
        while (start < end) {
            double t = a[start];
            a[start++] = a[end];
            a[end--] = t;
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (double) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (double) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final double[] test = new double[MAX_N];
    private static final double[] golden = new double[MAX_N];
