...
public final class MainFloat {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = float, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
...
*/
    ...
    private static void reverse(float[] a, int start, int end) {
        while (start < end) {
            float t = a[start];
            a[start++] = a[end];
            a[end--] = t;
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (float) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (float) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final float[] test = new float[MAX_N];
    private static final float[] golden = new float[MAX_N];
