...
public final class MainShort {

    public static void main(String[] args) {
        Locale.setDefault(new Locale("us", "US"));

        System.out.println("# Data type = short, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("jdk", new Sortable() { public void sortArray(short[] a) { DualPivotQuicksort_jdk.sort(a, parallelism, LEFT, HIGH ); }});
        doSort("__ ", new Sortable() { public void sortArray(short[] a) { DualPivotQuicksort    .sort(a,              LEFT, HIGH ); }});
...
*/
    ...
    private static void reverse(short[] a, int start, int end) {
        while (start < end) {
            short t = a[start];
            a[start++] = a[end];
            a[end--] = t;
    ...
    private static void stagger(int m) {
        for (int i = 0; i < MAX_N; ++i) {
            golden[i] = (short) ((i * m + i) % MAX_N);
        }
    }

    private static void shuffle(int m) {
        for (int i = 0, j = 0, k = 1; i < MAX_N; ++i) {
            golden[i] = (short) (random.nextInt(m) > 0 ? (j += 2) : (k += 2));
        }
    }

    public static void check(String name, short[] a1, short[] a2, int left, int right) {
        String message;
    ...
    public static void outArr(short[] a) {
        outArr(a, 0, a.length - 1);
    }

    public static void outArr(short[] a, int left, int right) {
        for (int i = left; i <= right; ++i) {
...
//  private static final int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static final short[] test = new short[MAX_N];
    private static final short[] golden = new short[MAX_N];
