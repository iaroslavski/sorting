...
public final class Main {
    ...
    public static void main(String[] args) {
        out("# Data type = float, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
    ...
    private static void doIteration(String name) {
        ...
        doSort("jdk", new Sortable() { public void sortArray(float[] a) { DualPivotQuicksort_jdk   .sort(a, parallelism, LEFT, HIGH ); }});
        doSort("__ ", new Sortable() { public void sortArray(float[] a) { DualPivotQuicksort       .sort(a, parallelism, LEFT, HIGH ); }});
...
//      new DPQKiller().kill(golden);
    ...
    private static void permutation() {
        ...
        for (int i = MAX_N; i > 1; i--) {
            ...
            float t = golden[i - 1]; golden[i - 1] = golden[k]; golden[k] = t;
        }
    }
    ...
    private static void reverse(float[] a, int start, int hi) {
        hi--;
        while (start < hi) {
            float tmp = a[start];
            a[start++] = a[hi];
    ...
    public static void outArr(String name, float[] a) {
        out("\n" + name);
        outArr(a);
    }

    public static void outArr(float[] a) {
        outArr(a, 0, a.length - 1);
    }

    public static void outArr(float[] a, int left, int right) {
    ...
    public static void outArr(float[] a, int left, int high, boolean b) {
    ...
    public static void check(String name, float[] a1, float[] a2, int left, int right) {
        check(name, a1, a2, left, right, false);
    }

    public static void check(String name, float[] a1, float[] a2, int left, int right, boolean show) {
        String message;
...
//  private static int parallelism = 0;
    ...
    private static float[] golden = new float[MAX_N];
