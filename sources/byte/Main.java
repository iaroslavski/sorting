...
public final class Main {
    ...
    public static void main(String[] args) {
        out("# Data type = byte, Sorting type = " + (parallelism == 0 ? "sequential" : "parallel " + (parallelism + 1)) + ", Length = " + MAX_N);
...
//      new DPQKiller().kill(golden);
    ...
    private static void outArr(byte[] a) {
    ...
    public static void copy(byte[] src, byte[] dst, int left, int right) {
        for (int i = left; i <= right; ++i) {
            dst[i] = src[i];
        }
    }
    ...
    public static void check(String name, byte[] a1, byte[] a2, int left, int right, boolean show) {
        for (int i = 0; i < LEFT; i++) {
...
//  private static int parallelism = ForkJoinPool.getCommonPoolParallelism();
    ...
    private static byte[] save = new byte[MAX_N];
    private static byte[] test = new byte[MAX_N];
